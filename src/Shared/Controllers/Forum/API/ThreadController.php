<?php
namespace Shared\Http\Controllers\Forum\API;
...
class ThreadController extends BaseController
{
	...
	public function create(Request $request)
	{
		...
		$thread = Thread::create($request->only(['category_id', 'author_id', 'title']));
	...
	{
		$thread = Thread::where('locked', 0)->find($id);
	...
	{
		$thread = Thread::where('locked', 1)->find($id);
	...
	{
		$thread = Thread::where('pinned', 0)->find($id);
	...
	{
		$thread = Thread::where('pinned', 1)->find($id);

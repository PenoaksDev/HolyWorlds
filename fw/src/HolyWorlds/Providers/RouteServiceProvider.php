...
use Penoaks\Providers\RouteServiceProvider as ServiceProvider;
...
class RouteServiceProvider extends ServiceProvider
{
	 ...
	 * @var string
	 */
	protected $namespace = 'HolyWorlds\Controllers';
	 ...
	 * @return void
	 */
	public function map( Router &$r )
	{
		/*
 * $fw->router->group( ['namespace' => 'HolyWorlds\Controllers'], function ( Router $router )
{
$r = &$router;
		...
		], function ( Router $r )
		{
			$r->get( '/', function ()
			{
				return view( 'index', [
					...
					'newThreads' => Thread::with( [
						'author',
						'posts'
					] )->orderBy( 'created_at', 'desc' )->limit( 5 )->get(),
					'newPosts' => Post::where( 'post_id', '!=', null )->orderBy( 'created_at', 'DESC' )->limit( 5 )->get(),
					'articles' => Article::published()->orderBy( 'published_at', 'desc' )->paginate( 15 )
				] );
			} );
			...
			$r->group( ['prefix' => 'auth'], function ( Router $r )
			{
				...
				$r->post( 'activate', [
					'as' => 'auth.post.activation',
					'uses' => 'Auth\AuthController@postActivation'
				] );
			...
			$r->group( ['prefix' => 'account', 'as' => 'account.'], function ( Router $r )
			{
				...
				$r->get( 'notifications', [
					'as' => 'notifications',
					'uses' => 'User\AccountController@getNotifications'
				] );
			...
			*/
			...
			$r->group( ['prefix' => 'admin', 'namespace' => 'Admin'], function ( Router $r )
			{
				...
				$r->get( "users/{userId}/groups", [
					"as" => "admin.users.groups",
					"uses" => "UserController@listGroups"
				] );
			...
			$r->group( ["prefix" => "forum", "namespace" => "Forum", "as" => "forum."], function ( Router $r )
			{
				...
				$r->group( ['prefix' => '{category}-{category_slug}'], function ( Router $r )
				{
					...
					$r->patch( '{thread}-{thread_slug}', [
						'as' => 'thread.update',
						'uses' => "ThreadController@update"
					] );
					$r->delete( '{thread}-{thread_slug}', [
						'as' => 'thread.delete',
						'uses' => "ThreadController@destroy"
					] );

					// Posts
					$r->get( '{thread}-{thread_slug}/post/{post}', [
						'as' => 'post.show',
						'uses' => "PostController@show"
					] );
					$r->get( '{thread}-{thread_slug}/reply', [
						'as' => 'post.create',
						'uses' => "PostController@create"
					] );
					$r->post( '{thread}-{thread_slug}/reply', [
						'as' => 'post.store',
				...
				$r->group( ['prefix' => 'category', 'as' => 'category.'], function ( Router $r )
				{
					...
					$r->patch( '{id}/make-private', [
						'as' => 'make-private',
						'uses' => 'CategoryController@makePrivate'
					] );

...
trait Taggable
{
	...
	public static function bootTaggable()
	{
		...
		static::$taggingUtility = new Util();
	}
	...
	{
		return $this->morphMany('HolyWorlds\Tagging\Model\Tagged', 'taggable')->with('tag');
	}
	...
	public function scopeWithAllTags( $query, $tagNames )
	{
		...
		{
			$tags = Tagged::where( 'tag_slug', call_user_func( $normalizer, $tagSlug ) )->where( 'taggable_type', $className )->lists( 'taggable_id' );
	...
	public function scopeWithAnyTag( $query, $tagNames )
	{
		...
		$tags = Tagged::whereIn( 'tag_slug', $tagNames )->where( 'taggable_type', $className )->lists( 'taggable_id' );
	...
	{
		return Tagged::distinct()->join( 'tagging_tags', 'tag_slug', '=', 'tagging_tags.slug' )->where( 'taggable_type', '=', ( new static )->getMorphClass() )->orderBy( 'tag_slug', 'ASC' )->get( [
				'tag_slug as slug',
				'tag_name as name',
				'tagging_tags.count as count'
			] );
	}

...
abstract class ApiController extends Controller
{
	...
	protected function response( $data, $message = "", $code = 200 )
	{
		...
		return ( request()->ajax() || request()->wantsJson() ) ? new JsonResponse( $message + compact( 'data' ), $code ) : new Response( $data, $code );
	}
	...
	protected function notFoundResponse()
	{
		...
		return ( request()->ajax() || request()->wantsJson() ) ? new JsonResponse( $content, 404 ) : new Response( $content, 404 );
	}
	...
	protected function buildFailedValidationResponse( Request $request, $errors )
	{
		...
		return ( $request->ajax() && !$request->pjax() || $request->wantsJson() ) ? new JsonResponse( $content, 422 ) : new Response( $content, 422 );
	}
}

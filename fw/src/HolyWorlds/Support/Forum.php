...
class Forum
{
	...
	public static function routes( Router $router )
	{
		...
		$router->group( ['prefix' => '{category}-{category_slug}'], function ( $router ) use ( $controllers )
		{
			...
			// Threads
			$router->get( '{thread}-{thread_slug}', [
				'as' => 'thread.show',
				'uses' => "{$controllers['thread']}@show"
			] );
			...
			$router->patch( '{thread}-{thread_slug}', [
				'as' => 'thread.update',
				'uses' => "{$controllers['thread']}@update"
			] );
			$router->delete( '{thread}-{thread_slug}', [
				'as' => 'thread.delete',
				'uses' => "{$controllers['thread']}@destroy"
			] );

			// Posts
			$router->get( '{thread}-{thread_slug}/post/{post}', [
				'as' => 'post.show',
				'uses' => "{$controllers['post']}@show"
			] );
			$router->get( '{thread}-{thread_slug}/reply', [
				'as' => 'post.create',
				'uses' => "{$controllers['post']}@create"
			] );
			$router->post( '{thread}-{thread_slug}/reply', [
				'as' => 'post.store',
				'uses' => "{$controllers['post']}@store"
			] );
			$router->get( '{thread}-{thread_slug}/post/{post}/edit', [
				'as' => 'post.edit',
				'uses' => "{$controllers['post']}@edit"
			] );
			$router->patch( '{thread}-{thread_slug}/{post}', [
				'as' => 'post.update',
				'uses' => "{$controllers['post']}@update"
			] );
			$router->delete( '{thread}-{thread_slug}/{post}', [
				'as' => 'post.delete',
				'uses' => "{$controllers['post']}@destroy"
			] );
		} );
